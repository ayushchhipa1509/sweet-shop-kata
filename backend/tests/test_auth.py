...
def test_register_existing_user(client: TestClient, session: Session):
    ...
    user = User(username="existinguser", email="existing@example.com", password_hash="somehash")
    session.add(user)
    session.commit()
...
def test_login_with_wrong_password(client: TestClient, session: Session):
    ...
    password = "password123"
    hashed_password = auth.get_password_hash(password)
    user = User(username="wrongpassuser", email="wrongpass@example.com", password_hash=hashed_password)
    session.add(user)
    session.commit()
